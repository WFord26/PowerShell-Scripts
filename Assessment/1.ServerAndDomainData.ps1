# PowerShell Server Assessment Step 1
# PowerShell assess a Windows server. See the installed roles
# and features, and the status of the services.


### VARIABLES ###
# Define the servers you want to assess
#$servers = import-csv $serverList | Select-Object -ExpandProperty ServerName
$servers = @("TSServer", "ServerLA", "VPS-LA-01", "DCLA", "BkupSrvLA-01", "AppVSrvLA-01", "Server-PRTG", "VFS-NC-01", "VFS-LA-01","HostSrv-LA01", "HostSrv-LA02", "HostSrv-LA03", "HostSrv-LA04", "HostSrv-LA05")
...
function Combine-ServerRawData {
    ...
    if ($null -ne $printerCSV) {
        $printerCSV | ForEach-Object {
            ...
            $printerData | Export-Csv -Path "combined-printers.csv" -Append -NoTypeInformation
            If ($logfile) {
    ...
    if ($null -ne $sharedFolderCSV) {
        $sharedFolderCSV | ForEach-Object {
            ...
            $sharedFolderData | Add-Member -MemberType NoteProperty -Name "Migrate" -Value "" -Force
            $sharedFolderData | Export-Csv -Path "combined-shared_folders.csv" -Append -NoTypeInformation
            if ($logfile) {
    ...
    if ($null -ne $websitesCSV) {
        $websitesCSV | ForEach-Object {
            ...
            $websitesData | Export-Csv -Path "combined-websites.csv" -Append -NoTypeInformation
            if ($logfile) {
    ...
    if ($null -ne $serverSoftwareCSV) {
        $serverSoftwareCSV | ForEach-Object {
            ...
            $serverSoftwareData | Export-Csv -Path "combined-software_Servers.csv" -Append -NoTypeInformation
            if ($logfile) {
    ...
    if ($null -ne $disksCSV) {
        $disksCSV | ForEach-Object {
            ...
            $disksData | Export-Csv -Path "combined-disks.csv" -Append -NoTypeInformation
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined disk data from $($_.FullName)"
            }
        }
    }
    $servicesCSV = Get-ChildItem -Filter "RawData\services*.csv"
    if ($null -ne $servicesCSV) {
        $servicesCSV | ForEach-Object {
            $servicesData = Import-Csv -Path $_.FullName
            # Get the Name of the server from the CSV file. Remove the services_ prefix and the .csv suffix
            $serverName = $_.Name -replace "services_", "" -replace ".csv", ""
            $servicesData | Add-Member -MemberType NoteProperty -Name "Server" -Value "$serverName" -Force
            $servicesData | Export-Csv -Path "combined-services.csv" -Append -NoTypeInformation -Force
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined service data from $($_.FullName)"
            }
        }
    }
    $rolesCSV = Get-ChildItem -Filter "RawData\roles*.csv"
    if ($null -ne $rolesCSV) {
        $rolesCSV | ForEach-Object {
            $rolesData = Import-Csv -Path $_.FullName
            $serverName = $_.Name -replace "roles_", "" -replace ".csv", ""
            $rolesData | Add-Member -MemberType NoteProperty -Name "Server" -Value "$serverName" -Force
            # Remove rows that match the Installed column with FALSE
            $rolesData = $rolesData | Where-Object { $_.Installed -eq "TRUE" }
            $rolesData | Export-Csv -Path "combined-roles.csv" -Append -NoTypeInformation
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined role data from $($_.FullName)"
            }
        }
    }
    $adcsExtensionsCSV = Get-ChildItem -Filter "RawData\adcs_extensions*.csv"
    if ($null -ne $adcsExtensionsCSV) {
        $adcsExtensionsCSV | ForEach-Object {
            $adcsExtensionsData = Import-Csv -Path $_.FullName
            $adcsExtensionsData | Export-Csv -Path "combined-adcs_extensions.csv" -Append -NoTypeInformation
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined ADCS extensions data from $($_.FullName)"
            }
        }
    }
    $adcsTemplatesCSV = Get-ChildItem -Filter "RawData\adcs_templates*.csv"
    if ($null -ne $adcsTemplatesCSV) {
        $adcsTemplatesCSV | ForEach-Object {
            $adcsTemplatesData = Import-Csv -Path $_.FullName
            $adcsTemplatesData | Export-Csv -Path "combined-adcs_templates.csv" -Append -NoTypeInformation
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined ADCS templates data from $($_.FullName)"
            }
        }
    }
    $adcsInfoCSV = Get-ChildItem -Filter "RawData\adcs_info*.csv"
    if ($null -ne $adcsInfoCSV) {
        $adcsInfoCSV | ForEach-Object {
            $adcsInfoData = Import-Csv -Path $_.FullName
            $adcsInfoData | Export-Csv -Path "combined-adcs_info.csv" -Append -NoTypeInformation
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined ADCS info data from $($_.FullName)"
            }
        }
    }
    $networkAdaptersCSV = Get-ChildItem -Filter "RawData\network_adapters*.csv"
    if ($null -ne $networkAdaptersCSV) {
        $networkAdaptersCSV | ForEach-Object {
            $networkAdaptersData = Import-Csv -Path $_.FullName
            $networkAdaptersData | Export-Csv -Path "combined-network_adapters.csv" -Append -NoTypeInformation
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined network adapter data from $($_.FullName)"
            }
        }
    }
    $vmCSV = Get-ChildItem -Filter "RawData\virtual_machines*.csv"
    if ($null -ne $vmCSV) {
        $vmCSV | ForEach-Object {
            $vmData = Import-Csv -Path $_.FullName
            $vmData | Select-Object VMName, VMid, ComputerName, ProcessorCount, MemoryAssigned, HardDrives, NetworkAdapters, Path, ConfigurationLocation | Export-Csv -Path "combined-virtual_machines.csv" -Append -NoTypeInformation -force
            if ($logfile) {
                Add-Content -Path $logfile -Value "Combined virtual machine data from $($_.FullName)"
            }
        }
    }
...
# Run the script
Start-WindowsServerAssessment -domain $domain -servers $servers